checkAccess {
# dmp-group-access.r
# verify that access is restricted to a designated group of users
#Get USER_ID for the input group name
  checkUserInput (*Group, $rodsZoneClient);
  *Query = select USER_ID where USER_NAME = '*Group';
  *Userid = "";
  foreach(*Row in *Query) {
    *Userid = *Row.USER_ID;
  }
  if(*Userid == "") {
    writeLine("stdout","Input group name *Group is unknown");
    fail;
  }
  else {writeLine("stdout","Group ID is *Userid");}
