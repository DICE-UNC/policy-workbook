checkDuplicates {
  racGlobalSet ();
# Policy59
# rac-checkDuplicates.r
# create an ordered list of Audit-Handle and count the number of values
  msiGetSystemTime (*Tim, "human");
  *Path = GLOBAL_ACCOUNT ++ "/*Archive/" ++ GLOBAL_ARCHIVES;
  writeLine ("stdout", "Check for duplicate handles for *Path on *Tim");
  *Q1 = select order(META_DATA_ATTR_VALUE), count(META_DATA_ATTR_ID) where COLL_NAME = *Path and META_DATA_ATTR_NAME = "Audit-Handle";
  foreach (*R1 in *Q1) {
    *Val = *R1.META_DATA_ATTR_VALUE;
    *Num = *R1.META_DATA_ATTR_ID;
    *N = int (*Num) /2;
    writeLine ("stdout", "  *N   *Val");
    if (*N > 1) {
      *Q2 = select DATA_NAME, COLL_NAME where META_DATA_ATTR_VALUE = *Val and META_DATA_ATTR_NAME = "Audit-Handle";
      foreach (*R2 in *Q2) { 
        *F = *R2.DATA_NAME;
        *C = *R2.COLL_NAME;
        writeLine ("stdout", "    *C/*F");
      }
    }
  }
  racSaveFile("Archive-IDCA", *Archive);
}
racGlobalSet = maing
