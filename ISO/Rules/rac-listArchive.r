...
# Policy77
# rac-listArchive.r
# List the contents of an archive and store results in Archive-ARA
  msiGetSystemTime (*Tim, "human");
  *Coll = GLOBAL_ACCOUNT ++ "/*Archive/" ++ GLOBAL_ARCHIVES;
  writeLine ("stdout", "List of *Coll contents on *Tim");
  *Q1 = select DATA_NAME, DATA_CREATE_TIME where COLL_NAME like "*Coll%";
  foreach (*R1 in *Q1) {
    *File = *R1.DATA_NAME;
    *Da = *R1.DATA_CREATE_TIME;
    *Dat = timestrf(datetime(double(*Da)), "%Y %m %d");
    writeLine ("stdout", "  *File created on *Dat");
  }
  racSaveFile ("Archive-ARA", *Archive);
}
racGlobalSet = maing
