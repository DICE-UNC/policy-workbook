listAuditEventInfrastructure {
  racGlobalSet ();
# rac-listAuditEventInfrastructure.r
# Policy81
# Rule to execute an external script to retrieve a list of audit events
# Search for events for a particular archive
  *Cmd = "script-name";
  *A1 = "script-input-argument";
  *Arg1 = execCmdArg(*A1);
  writeLine ("stdout", "Executing script *Cmd"):
  if (errorcode(msiExecCmd(*Cmd, "*Arg1", "null", "null", "null", *Result)) < 0) {
  ...
  racSaveFile ("Archive-IRA", *Archive);
}
racGlobalSet = maing
