...
# Policy65
# rac-createAudit.r
# query ElasticSearch for actions on *Archive collection
  msiGetSystemTime(*Tim, "human");
  writeLine ("stdout", "Generate audit report for *Archive on *Tim");
  *Coll = GLOBAL_ACCOUNT ++ "/*Archive";
  *Q1 = select AUDIT_EVENT where COLL_NAME like "*Coll%";
  foreach (*R1 in *Q1) {
    *Event = *R1.AUDIT_EVENT;
    writeLine ("stdout", "  *Event");
  }
  racSaveFile("Archive-ARUA", *Archive);
}
racGlobalSet = maing
