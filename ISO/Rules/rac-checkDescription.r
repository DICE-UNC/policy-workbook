checkDescription {
  racGlobalSet ();
# Policy79
# rac-checkDescription.r
# check for the presence of Audit-Description on each AIP
  msiGetSystemTime (*Tim, "human");
  *Coll = GLOBAL_ACCOUNT ++ "/*Archive/" ++ GLOBAL_ARCHIVES;
  writeLine ("stdout", "Checked for presence of Audit-Description on each AIP in *Archive on *Tim");
  *Q1 = select DATA_NAME, COLL_NAME where COLL_NAME like "*Coll%";
  foreach (*R1 in *Q1) {
    *File = *R1.DATA_NAME;
    *Col = *R1.COLL_NAME;
    *Q2 = select count(META_DATA_ATTR_ID) where DATA_NAME = *File and COLL_NAME = *Col and META_DATA_ATTR_NAME = "Audit-Description";
    foreach (*R2 in *Q2) {
      *Num = *R2.META_DATA_ATTR_ID;
      if (*Num == "0") {
        writeLine ("stdout", "  *Col/*File is missing Audit-Description");
      }
    }
  }
  racSaveFile ("Archive-MA", *Archive);
}
racGlobalSet = maing
