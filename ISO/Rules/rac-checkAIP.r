...
# Policy54
# rac-checkAIP.r
# verify each AIP has an Audit-Comply flag set to 1
  *Coll = GLOBAL_ACCOUNT ++ "/*Archive/" ++ GLOBAL_AIPS;
  msiGetSystemTime (*Tim, "human");
  writeLine ("stdout", "Checked AIPS in *Coll for compliance at *Tim");
  *Q1 = select DATA_NAME, COLL_NAME where COLL_NAME like "*Coll%";
  foreach (*R1 in *Q1) {
    *C = *R1.COLL_NAME;
    *F = *R1.DATA_NAME;
    *P = "*C/*F";
    *Q2 = select count(META_DATA_ATTR_VALUE) where DATA_NAME = *F and COLL_NAME = *C and META_DATA_ATTR_NAME = "Audit-Comply";
    foreach (*R2 in *Q2) {
      *N = *R2.META_DATA_ATTR_VALUE;
      if (*N == "0") {
        writeLine ("stdout", "Non-compliant AIP, *C/*V");
      }
    }
  }
  racSaveFile ("Archive-AIPCRA", *Archive);
}
