allowAccess {
# dmp-publication.r
# The rule sets the PUBLICATION flag to 1 on files in a collection
    ...
    foreach(*R2 in *Q2) {
      ...
      *Val = *R2.META_DATA_ATTR_VALUE;
      if(*Name == "PUBLICATION") {
        *Count = *Count + 1;
        if(*Val != "1") {
# remove old PUBLICATION flag
          deleteAVUMetadata (*Path, "PUBLICATION", *Val, "", *Status);
# Set PUBLICATION flag to 1
          if (*Count == 1) {
            addAVUMetadata (*Path, "PUBLICATION", "1", "", *Status);
            writeLine("stdout", "Set PUBLICATION for *Coll1/*File");
          }
        }
      }
    }
